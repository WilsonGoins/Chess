...
void WelcomeScreen(sf::RenderWindow& window, Images& images, string& user1, string& user2, bool& user1White) {
    ...
    welcomeText.setPosition(sf::Vector2f(150, 150));        // put it in the top left
    // draw player 1 prompt
    sf::Text player1Prompt("Player 1:                            ", font, 40);      // lots of extra space so the clickable area is bigger
    player1Prompt.setFillColor(sf::Color::White);
    player1Prompt.setPosition(sf::Vector2f(710, 100));
    // draw player 2 prompt
    sf::Text player2Prompt("Player 2:                            ", font, 40);
    player2Prompt.setFillColor(sf::Color::White);
    player2Prompt.setPosition(sf::Vector2f(710, 175));
    // player 1 name text
    string p1name = "P1";
    sf::Text player1Name(p1name, font, 40);
    ...
    // player 2 name text
    string p2name = "P2";
    sf::Text player2Name(p2name, font, 40);
    ...
    string userName = p1name;           // username to save code by not having to switch between p1name and p2name

    while (window.isOpen()) {
        // do these in main loop, so it constantly happens
        window.draw(welcomeImage);      // draw background image
        window.draw(welcomeText);           // draw "CHESS"
        window.draw(player1Prompt);             // draw name prompt
        window.draw(player2Prompt);             // draw other name prompt
        player1Name.setString(p1name);              // set the string of player 1 to be the updated player1 name
        ...
        player2Name.setString(p2name);              // set the string of player 2 to be the updated player2 name
        ...
        while (window.pollEvent(event)) {
            if (event.type == sf::Event::Closed) {      // if they close the window
                ...
                if (player1Prompt.getGlobalBounds().contains(click.x, click.y)) {     // if they click player1's name
                    cout << click.x << endl;
                    P1Selected = true;          // player1 is selected
                    userName = p1name;          // username equals the current player 1 name
                } else if (player2Prompt.getGlobalBounds().contains(click.x, click.y)) {
                    P1Selected = false;         // player2 is selected
                    userName = p2name;          // set username equal to player2 name
                } else if (playButton.getGlobalBounds().contains(click.x, click.y)) {
                    user1 = p1name;
                    user2 = p2name;
                    user1White = player1White;
                    return;
                }
